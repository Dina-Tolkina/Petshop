...
2.  Настройка базы данных:
    ...
    ```sh
      ...
      DB_PORT=5432
    ```
3. Запуск команды для создания media папки 
    ```sh
      docker exec -it petshop-web-1 bash -c "mkdir -p /app/media/customer && mkdir -p /app/media/products"
    ```
4. Запуск команды для создания суперпользователя:
   ```sh
